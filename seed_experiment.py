...
def seed_experiment(dir_name, curriculum_bool, bucket_criterion, num_units, num_curr, num_patience, dataset, seed_continue):
    ...
    experiment_dir = os.path.join(higher_wd,dir_name)
    ...
    for i in seeds:
        ...
        start = time.time()
        model, history, num_data_pts_list, test_history, train_til_end, when_added = baby_step_loss_acc.train_baby_step(x_train_val, y_train_val, x_test, y_test, args_dict) # 이제는 이게 best model 이 아님
    ...
    return train_til_end_list, avg_acc